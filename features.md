...
- Interaction gate: `AFirstPersonCharacter::RaycastInteractableCheck` also uses this sphere as the NPC interaction range. After the LoS check passes, if the hit actor has a `ULookAtPlayerComponent` it must also be overlapping the player (`IsOverlappingActor(this)`), otherwise the hit is rejected. This keeps "I can interact with this NPC" and "this NPC is looking at me" in lockstep, and honors per-instance sphere radius overrides set in the editor. Non-NPC interactables (doors, movie boxes) are unaffected and still use the full `InteractionDistance`.
