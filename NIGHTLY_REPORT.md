...
- 🟡 **First-play fog wall + bladder indicator** — **UPDATED after live morning debugging with you (2026-07-02)**, which corrected the overnight hypotheses:
  - **Fog wall (`ExponentialHeightFog`, the black horizon band):** on the session's first world after editor launch, the fog's *render proxy never gets created* (PSO-precache delayed-proxy-creation loses its ready callback on the first scene). Proven live: in your broken play, density 10×, gray inscattering, volumetric toggles and cvar/show-flag forcing all changed **nothing on screen** while every value read healthy — and the identical values rendered fine on the next world. Fix: `StorySubsystem` rebuilds the fog's render state 1s into gameplay (no-op when healthy).
