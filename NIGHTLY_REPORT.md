...
- 🟡 **First-play fog wall + bladder indicator** — **UPDATED after live morning debugging with you (2026-07-02)**, which corrected the overnight hypotheses:
  - **Fog wall (`ExponentialHeightFog`, the black horizon band): still open — and we learned the hard way what NOT to do.** Live facts: in a broken play, density 10×, gray inscattering, volumetric toggles and cvar/show-flag forcing all changed **nothing on screen** while every value read healthy; identical values render fine on any new world. I shipped a "rebuild the fog render state at world start" guard — it **regressed play 2** (mid-play recreation of the fog render state deterministically produces dead fog) and was reverted the same hour. That regression is the best clue yet: the first-play breakage is almost certainly *something recreating the fog render state early in the session's first play* (prime suspect: a first-run-only init branch in UDS applying fog settings through state-dirtying setters). Next probes when you're up for one more session: a two-frame visibility cycle in a broken play, and an `r.PSOPrecache.ProxyCreationWhenPSOReady=0` config experiment.
