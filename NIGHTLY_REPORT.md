...
2. **The TVs blare a looping tornado-alert siren** — `/Game/Sounds/tornadoalert` (CC0, *EAS Alarm* by mpaol2023 — credited, 12.84s clip) plays diegetically from each TV over the warning screen. The wave is a **one-shot**; `ACRTTV` re-fires it after a **designer-set `WarningLoopGapSeconds`** silent gap (default 1.5s) so it doesn't loop instantly/seamlessly — tune the gap on `BP_TV` defaults. **Confined to the store via the `AV_VideoStore` audio volume:** the siren routes through the `SC_Ambient` SoundClass (`bApplyAmbientVolumes`) and the volume's **`interior_volume` is set to `0`** (store-interior sounds silenced when the listener is outside, 0.5s fade) — `exterior_volume` is left at `0.6` so the outside world is still heard *inside* the store. So the siren is audible in the store and silenced once you cross the boundary, nowhere else. (Occlusion was tried first but interior shelves/movieboxes false-occluded the line trace; the volume needs no traces.)
